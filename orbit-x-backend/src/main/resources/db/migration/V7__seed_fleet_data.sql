...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.datacenters WHERE name = 'Orbit X - Frankfurt Prime');

INSERT INTO RM559810.datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active, created_at)
SELECT 'Orbit X - Singapore Hub', 'Singapore', 'Singapore', 1.3521, 103.8198, 'STABLE', 19750.00, 5100, 1, SYSTIMESTAMP
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.datacenters WHERE name = 'Orbit X - Singapore Hub');

INSERT INTO RM559810.datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active, created_at)
SELECT 'Orbit X - New York Edge', 'New York', 'United States', 40.7128, -74.0060, 'OPTIMAL', 24300.00, 7200, 1, SYSTIMESTAMP
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.datacenters WHERE name = 'Orbit X - New York Edge');

INSERT INTO RM559810.datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active, created_at)
SELECT 'Orbit X - Tokyo Nexus', 'Tokyo', 'Japan', 35.6762, 139.6503, 'CRITICAL', 16900.00, 3900, 1, SYSTIMESTAMP
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.datacenters WHERE name = 'Orbit X - Tokyo Nexus');
...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.satellites WHERE name = 'OX-SAT-02 Helios');

INSERT INTO RM559810.satellites (name, orbit_type, altitude_km, inclination_deg, orbital_period_min, data_link_status, active, launched_at)
SELECT 'OX-SAT-03 Kronos', 'MEO', 8000.0, 55.0, 285.0, 'ACTIVE', 1, TIMESTAMP '2023-09-10 08:00:00'
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.satellites WHERE name = 'OX-SAT-03 Kronos');

INSERT INTO RM559810.satellites (name, orbit_type, altitude_km, inclination_deg, orbital_period_min, data_link_status, active, launched_at)
SELECT 'OX-SAT-04 Aurora', 'LEO', 530.0, 97.6, 95.0, 'DEGRADED', 1, TIMESTAMP '2024-01-22 18:00:00'
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.satellites WHERE name = 'OX-SAT-04 Aurora');

INSERT INTO RM559810.satellites (name, orbit_type, altitude_km, inclination_deg, orbital_period_min, data_link_status, active, launched_at)
SELECT 'OX-SAT-05 Zenith', 'GEO', 35786.0, 0.0, 1436.1, 'ACTIVE', 1, TIMESTAMP '2022-11-05 12:00:00'
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM RM559810.satellites WHERE name = 'OX-SAT-05 Zenith');
