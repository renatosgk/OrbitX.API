-- ============================================================
--  Orbit X Platform — Flyway V2 — Seed initial fleet data
--  Safe to re-inspect: inserts are guarded by NOT EXISTS.
-- ============================================================

-- ── Datacenters ────────────────────────────────────────────────
INSERT INTO datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active)
SELECT 'Orbit X - Sao Paulo Alpha', 'Sao Paulo', 'Brazil', -23.5505, -46.6333, 'STABLE', 18500.00, 4200, 1
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM datacenters WHERE name = 'Orbit X - Sao Paulo Alpha');

INSERT INTO datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active)
SELECT 'Orbit X - Frankfurt Prime', 'Frankfurt', 'Germany', 50.1109, 8.6821, 'OPTIMAL', 22100.00, 6800, 1
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM datacenters WHERE name = 'Orbit X - Frankfurt Prime');

INSERT INTO datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active)
SELECT 'Orbit X - Singapore Hub', 'Singapore', 'Singapore', 1.3521, 103.8198, 'STABLE', 19750.00, 5100, 1
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM datacenters WHERE name = 'Orbit X - Singapore Hub');

INSERT INTO datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active)
SELECT 'Orbit X - New York Edge', 'New York', 'United States', 40.7128, -74.0060, 'OPTIMAL', 24300.00, 7200, 1
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM datacenters WHERE name = 'Orbit X - New York Edge');

INSERT INTO datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active)
SELECT 'Orbit X - Tokyo Nexus', 'Tokyo', 'Japan', 35.6762, 139.6503, 'CRITICAL', 16900.00, 3900, 1
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM datacenters WHERE name = 'Orbit X - Tokyo Nexus');

INSERT INTO datacenters (name, city, country, latitude, longitude, thermal_state, regional_consumption_kwh, capacity_servers, active)
SELECT 'Orbit X - London Core', 'London', 'United Kingdom', 51.5074, -0.1278, 'STABLE', 20400.00, 5600, 1
